...
object Consts {
  ...
  val MEN_S   = 1.U(MEN_LEN.W) // scalar instruction
  val MEN_V   = 2.U(MEN_LEN.W) // vector instruction

  val REN_LEN = 2
  val REN_X   = 0.U(REN_LEN.W)
  val REN_S   = 1.U(REN_LEN.W) // scalar instruction
  val REN_V   = 2.U(REN_LEN.W) // vector instruction
